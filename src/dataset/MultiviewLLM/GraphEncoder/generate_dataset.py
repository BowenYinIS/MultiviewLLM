"""
Purpose:
    Generate a dataset for multiview LLM graph encoding tasks.

Input:
    `transaction` data: from `data/raw_data/sample_transaction.feather`
    `sample_index` data: from `data/processed_data/sample_index`
    `encoder` option: a encoder to encode mcc_cde. If None, will create a new one.

Output:
    A dataset saved in `data/processed_data/MultiviewLLM/GraphEncoder` containing:
        - A PyTorch Geometric dataset with graph data objects for each sample.

    A JSON file saved in `data/processed_data/MultiviewLLM/GraphEncoder` containing:
        - The mcc_cde encoder and decoder mappings.
"""
